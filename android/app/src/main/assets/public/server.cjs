...
function readProducts() {
  try {
    ...
    if (!import_fs.default.existsSync(PRODUCTS_FILE)) {
      import_fs.default.writeFileSync(PRODUCTS_FILE, JSON.stringify(INITIAL_PRODUCTS, null, 2), "utf-8");
...
function writeProducts(products) {
  try {
    ...
    import_fs.default.writeFileSync(PRODUCTS_FILE, JSON.stringify(products, null, 2), "utf-8");
    return true;
  } catch (err) {
...
async function optimizeBase64Image(dataUrl) {
  if (!dataUrl || typeof dataUrl !== "string" || !dataUrl.startsWith("data:image") || dataUrl.length < 25e4) {
    return dataUrl;
  }
  try {
    ...
    if (matches) {
      ...
      const compressed = await sharp(buffer).resize(900, 900, { fit: "inside", withoutEnlargement: true }).jpeg({ quality: 80, mozjpeg: true }).toBuffer();
...
app.post("/api/products", async (req, res) => {
  try {
    ...
    const newProduct = {
      ...
      weightOrSize: weightOrSize ? String(weightOrSize).trim() : void 0,
      flavour: flavour ? String(flavour).trim() : void 0,
...
app.put("/api/products/:id", async (req, res) => {
  try {
    ...
    let updatedImageUrl = req.body.imageUrl !== void 0 ? req.body.imageUrl : existing.imageUrl;
    ...
    const updated = {
      ...
      originalPrice: req.body.originalPrice !== void 0 ? Number(req.body.originalPrice) : existing.originalPrice,
      updatedAt: (/* @__PURE__ */ new Date()).toISOString()
    };
